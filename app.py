...
## Take image input and return base64 code of image
...
if st.session_state.metadata :
    ...
    with st.container():
        st.session_state.search_params['search_mode'] = st.radio('Search Mode : ',('Any of the selected classes (OR)','All selected classes (AND)'),horizontal=True)
        ...
        if st.session_state.search_params['selected_classes']:
            ...
            for i,cls in enumerate(st.session_state.search_params['selected_classes']):
                with cols[i]:
                    st.session_state.search_params['thresholds'][cls] = st.selectbox(f'Max count for {cls}',
        ...
        if st.button('Search Images',type='primary') and st.session_state.search_params['selected_classes']:
            ...
            for item in st.session_state.metadata:
                ...
                if search_params['search_mode'] == 'Any of the selected classes (OR)' : # or
...
if st.session_state.search_results:
    ...
    with st.expander('Display Options',expanded=True):
        ...
        with cols[2]:
            st.session_state.highight_matches = st.checkbox('Highlight matching classes',value=st.session_state.highight_matches)
                ...
                if st.session_state.show_boxes:
                    ...
                    for det in result['detections']:
                        ...
                        if cls in search_params['selected_classes']:
                            color = "#09FFA1"
                            thikness = 3
                        elif not st.session_state.highight_matches:
                            color = '#666666'
                            thikness = 1
                        else:
                            continue
                        draw.rectangle(bbox,outline=color,width=thikness)
                        
                        if cls in search_params['selected_classes'] or not  st.session_state.highight_matches:
